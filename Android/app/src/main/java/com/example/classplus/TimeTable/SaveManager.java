package com.example.classplus.TimeTable;

import com.example.classplus.TimeTable.Schedule;
import com.example.classplus.TimeTable.Sticker;
...
public class SaveManager {
    public static String saveSticker(HashMap<Integer, com.example.classplus.TimeTable.Sticker> stickers){
        ...
        for(int i = 0 ; i < orders.length; i++){
            ...
            ArrayList<com.example.classplus.TimeTable.Schedule> schedules = stickers.get(idx).getSchedules();
            for(com.example.classplus.TimeTable.Schedule schedule : schedules){
    ...
    public static HashMap<Integer, com.example.classplus.TimeTable.Sticker> loadSticker(String json){
        HashMap<Integer, com.example.classplus.TimeTable.Sticker> stickers = new HashMap<Integer, com.example.classplus.TimeTable.Sticker>();
        JsonParser parser = new JsonParser();
        JsonObject obj1 = (JsonObject)parser.parse(json);
        JsonArray arr1 = obj1.getAsJsonArray("sticker");
        for(int i = 0 ; i < arr1.size(); i++){
            com.example.classplus.TimeTable.Sticker sticker = new com.example.classplus.TimeTable.Sticker();
            JsonObject obj2 = (JsonObject)arr1.get(i);
            int idx = obj2.get("idx").getAsInt();
            JsonArray arr2 = (JsonArray)obj2.get("schedule");
            for(int k = 0 ; k < arr2.size(); k++){
                com.example.classplus.TimeTable.Schedule schedule = new Schedule();
                ...
                com.example.classplus.TimeTable.Time startTime = new com.example.classplus.TimeTable.Time();
                ...
                com.example.classplus.TimeTable.Time endTime = new Time();
